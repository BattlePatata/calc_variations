...
txt_display = tk.Entry(calc_frame, 
                       width = 30,
...
def num_btn_maker(num, row_, col):
    tk.Button(calc_frame,
              ...
              text = num, 
              command = lambda: added_val.numEnter(num)).grid(
                  row = row_,
                  column = col, 
                  pady = 1)
...
def symb_btn_maker(text_, cmd, row_, col):
    tk.Button(calc_frame, 
              ...
              bg = "#FF9F0A",
              font = ("Helvetica", 20, "bold"),
              command = cmd).grid(row = row_, column = col)
...
zero_btn = tk.Button(calc_frame, text = "0",
                     width = 6, height = 1,
                     bg = "#333333",
                     fg = "white",
                     font = ("Helvetica", 20, "bold"),
                     command = added_val.numEnter(0))
...
symb_btn_maker("=", lambda: added_val.total_sum, 5, 3) # Button Equal
...
symb_btn_maker("Mod", added_val.operation("mod"), 3, 6) # Button Mod
...
lbl_display = tk.Label(calc_frame, text = "Scientific Calculator",
                       font = ("Helvetica", 20, "bold"),
...
def Scientific():
    app.resizable(0, 0)
    app.geometry("903x332+0+0")

def Standart():
    app.resizable(0, 0)
    app.geometry("454x332+0+0")
